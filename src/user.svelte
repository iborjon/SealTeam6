<script>
  ...
  async function handleLogin(e) {
    ...
    if (!userInitials.trim() || userInitials.length !== 3) {
      errorMessage = 'Please enter valid initials (exactly 3 characters).';
      isLoading = false;
      return;
    }
    ...
    localStorage.setItem('user', JSON.stringify({ name: userName, initials: userInitials }));
  }
  ...
  onMount(() => {
    ...
    if (savedUser) {
      const { name, initials } = JSON.parse(savedUser);
      userName = name;
                ...
                {#each projects as project (project.id)}
                  <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      <button 
                        on:click={() => alert(`Opening project: ${project.name}`)}
                      ...
                      {#if isLeadAnalyst && project.leadAnalyst === userInitials}
                        ...
                        <button 
                          on:click={() => alert(`Locking project: ${project.name}`)}
                  ...
                  <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                      ...
                      <button 
                        ...
                        on:click={() => copyToClipboard(project.port)}
                      >
                        Copy Port
                      </button>
                    </td>
